import { getIssue } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/getIssue.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getIssue(issueId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
