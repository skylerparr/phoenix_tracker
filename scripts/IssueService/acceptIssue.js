import { acceptIssue } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/acceptIssue.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  acceptIssue(issueId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
