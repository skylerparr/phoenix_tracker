import { finishIssue } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/finishIssue.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  finishIssue(issueId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
