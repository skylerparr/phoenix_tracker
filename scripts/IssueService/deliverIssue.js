import { deliverIssue } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/deliverIssue.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  deliverIssue(issueId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
