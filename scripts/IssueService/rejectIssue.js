import { rejectIssue } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/rejectIssue.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  rejectIssue(issueId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
