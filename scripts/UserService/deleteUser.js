import { deleteUser } from './user_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('UserService/deleteUser.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  deleteUser(userId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
    .then(() => prettyPrint({ message: 'User deleted successfully' }, config.output))
