import { getAllProjectsByUserId } from './project_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('ProjectService/getAllProjectsByUserId.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getAllProjectsByUserId(token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
