import { switchProject } from './auth_service.js';
import { parseArgs, prettyPrint, die } from '../cli.js';

if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('AuthService/switchProject.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });

  const projectId = process.env.PROJECT_ID ? parseInt(process.env.PROJECT_ID, undefined) : extractArgValue('--project-id');
  ...
  switchProject(projectId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
