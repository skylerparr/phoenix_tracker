import { getIssuesByUser } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/getIssuesByUser.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  if (!userId) die('User ID is required. Set USER_ID env var or use --user-id flag.');
  if (!token) die('Token is required. Set TOKEN env var or use --token flag.');

  getIssuesByUser(userId, token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
