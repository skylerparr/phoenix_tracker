...
export const useDragDropUpload = ({
  ...
  const validateFiles = useCallback((files: File[]): { valid: File[]; errors: string[] } => {
    ...
    if (files.length > maxFiles) {
      validationErrors.push(`Maximum ${maxFiles} files allowed. You tried to upload ${files.length} files.`);
    ...
    files.forEach((file) => {
      ...
      if (file.size > maxFileSize) {
        validationErrors.push(`${file.name}: File size exceeds maximum allowed size of ${Math.round(maxFileSize / (1024 * 1024))}MB`);
        return;
      }
  ...
  const uploadFiles = useCallback(async (files: File[]) => {
    ...
    try {
      const uploaded = await uploadService.uploadMultipleForIssue(issueId, validFiles);
