import { getAllUsers } from './user_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('UserService/getAllUsers.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getAllUsers(token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
