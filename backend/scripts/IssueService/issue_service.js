...
export async function createIssue(title, priority, status, isIcebox, workType, token, options = {}) {
  ...
  if (!title || priority === undefined || status === undefined || isIcebox === undefined || workType === undefined) {
    throw new Error('Title, priority, status, isIcebox, and workType are required');
  }
  ...
  const payload = JSON.stringify({
    ...
    isIcebox: isIcebox === 'true' || isIcebox === true,
    workType: parseInt(workType, 10),
    description: options.description,
    points: options.points ? parseInt(options.points, 10) : null,
