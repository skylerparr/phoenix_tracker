import { getAllAccepted } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/getAllAccepted.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getAllAccepted(token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
