import { getMyIssues } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/getMyIssues.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getMyIssues(token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
