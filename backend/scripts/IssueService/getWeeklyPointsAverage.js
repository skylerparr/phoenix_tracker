import { getWeeklyPointsAverage } from './issue_service.js';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = parseArgs('IssueService/getWeeklyPointsAverage.js', {
    includeBaseUrl: true,
    includeOutput: true,
  });
  ...
  getWeeklyPointsAverage(token, { baseUrl: config.baseUrl, insecure: config.insecure, cacert: config.cacert })
